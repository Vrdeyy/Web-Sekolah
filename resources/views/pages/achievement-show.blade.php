        ...
        <div class="container mx-auto px-4 lg:px-8 relative z-10">
            <nav class="flex flex-wrap items-center gap-2 text-xs md:text-[10px] font-extrabold uppercase tracking-[0.2em]"
                ...
                <a href="{{ route('achievements') }}"
                    class="text-gray-400 hover:text-[#F3DCEB] transition-colors tracking-widest">PRESTASI</a>
                <i class="fas fa-chevron-right text-[8px] text-gray-700"></i>
                <span
                    class="text-[#F3DCEB] font-extrabold line-clamp-1 break-all">{{ \Illuminate\Support\Str::limit(strtoupper($achievement->title), 30) }}</span>
            </nav>
        </div>
    </section>
                        ...
                        <div class="flex flex-wrap items-center gap-8 pb-10 border-b border-gray-200" data-aos="fade-up"
                            ...
                            @if($achievement->competition_name)
                                <div class="flex items-center gap-4 group" data-aos="fade-right" data-aos-delay="700">
                                    <div
                                        ...
                                        <i
                                            class="fas fa-calendar-star text-[#932F80] group-hover:text-white transition-colors"></i>
                                    </div>
                                    <div class="flex flex-col">
                                        <span
                                            class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Event</span>
                                        <span class="text-[#2A1424] font-extrabold">{{ $achievement->competition_name }}</span>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($achievement->year)
                                <div class="flex items-center gap-4 group" data-aos="fade-right" data-aos-delay="800">
                                    ...
                                    <div class="flex flex-col">
                                        <span
                                            class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Tahun</span>
                                        <span class="text-[#2A1424] font-extrabold">{{ $achievement->year }}</span>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($achievement->rank)
                                <div class="flex items-center gap-4 group" data-aos="fade-right" data-aos-delay="900">
                                    <div
                                        class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center border border-gray-100 group-hover:bg-purple-600 transition-colors duration-500 shadow-lg shadow-purple-900/5">
                                        <i class="fas fa-medal text-purple-500 group-hover:text-white transition-colors"></i>
                                    </div>
                                    <div class="flex flex-col">
                                        <span
                                            class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Juara</span>
                                        <span class="text-[#2A1424] font-extrabold">{{ $achievement->rank }}</span>
                                    </div>
                                </div>
                            @endif
                        </div>
                    ...
                    @if($achievement->image)
                        <figure
                            ...
                            <img src="{{ asset('storage/' . $achievement->image) }}" alt="{{ $achievement->title }}"
                                ...
                                onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($achievement->title) }}&background=932F80&color=fff&size=800'">
                        </figure>
                    @endif
                ...
                <aside class="lg:col-span-4 space-y-8 md:space-y-12">
                    ...
                    <div class="bg-white border border-gray-200 rounded-3xl md:rounded-[2.5rem] p-6 md:p-8 shadow-xl shadow-purple-900/5"
                        ...
                        <h3 class="font-extrabold text-[#2A1424] text-xl mb-8 flex items-center gap-3 tracking-tight">
                            <i class="fas fa-award text-[#932F80]"></i>
                            Prestasi Lainnya
                        </h3>
                        <div class="space-y-6">
                                    ...
                                    <div
                                        ...
                                        @else
                                            <div class="w-full h-full flex items-center justify-center bg-[#932F80]/10">
                                                <i
                                                    class="fas fa-trophy text-[#932F80] group-hover:scale-125 transition-transform"></i>
                                            </div>
                                        @endif
                                    </div>
                                    <div class="flex-1 min-w-0 flex flex-col justify-center">
                                        <h4
                                            class="font-extrabold text-[#2A1424] text-sm line-clamp-2 group-hover:text-[#932F80] transition-colors leading-snug tracking-tight mb-1">
                                            {{ $item->title }}
                                        </h4>
                                        <span
                                            class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{{ $item->date ? \Carbon\Carbon::parse($item->date)->format('Y') : '' }}</span>
                                    </div>
                                </a>
                            @endforeach
                        </div>
