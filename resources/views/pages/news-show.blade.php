...
@section('content')
    ...
    <section class="pt-28 pb-8 bg-gradient-to-br from-[#1A0E17] via-[#2A1424] to-[#12080F] relative overflow-hidden flex items-center">
        ...
        <div class="container mx-auto px-4 lg:px-8 relative z-10">
            <nav class="flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.2em]">
        ...
        <div class="container mx-auto px-4 lg:px-8">
            <div class="grid lg:grid-cols-12 gap-12">
                ...
                <article class="lg:col-span-8">
                    ...
                    <header class="mb-12">
                        @if($news->category)
                            <a href="{{ route('news') }}?category={{ $news->category }}"
                                class="inline-flex items-center gap-2 px-6 py-2.5 bg-[#932F80]/10 text-[#932F80] text-[10px] font-extrabold rounded-xl hover:bg-[#932F80] hover:text-white transition-all duration-500 mb-8 border border-[#932F80]/20 uppercase tracking-widest shadow-glow">
                                <i class="fas fa-folder text-[10px]"></i>
                                {{ $news->category }}
                            </a>
                        @endif

                        <h1 class="text-4xl md:text-5xl lg:text-7xl font-extrabold text-[#2A1424] leading-[1.1] mb-10 tracking-tight drop-shadow-sm">
                            {{ $news->title }}
                        </h1>

                        {{-- Meta Info --}}
                        <div class="flex flex-wrap items-center gap-8 pb-10 border-b border-gray-200">
                    ...
                    {{-- Featured Image --}}
                    @if($news->image)
                        <figure class="mb-14 rounded-[3rem] overflow-hidden shadow-2xl shadow-purple-900/10 border border-purple-100 group">
                    ...
                    {{-- Tags --}}
                    @if($news->tags)
                        <div class="mb-14 pb-12 border-b border-gray-200">
                            <h4 class="text-[10px] font-extrabold text-gray-500 uppercase tracking-[0.3em] mb-8">TAGS</h4>
                            <div class="flex flex-wrap gap-4">
                                @foreach(explode(',', $news->tags) as $tag)
                                    <span
                                        class="px-6 py-3 bg-white text-gray-500 text-xs font-bold rounded-2xl border border-gray-200 hover:bg-[#932F80]/10 hover:text-[#932F80] hover:border-[#932F80]/30 transition-all cursor-pointer uppercase tracking-widest shadow-sm">
                                        #{{ trim($tag) }}
                                    </span>
                                @endforeach
                            </div>
                    ...
                    {{-- Share Buttons --}}
                    <div class="bg-gradient-to-br from-white to-purple-50 border border-purple-100 rounded-[3rem] p-10 md:p-14 mb-14 relative overflow-hidden group shadow-xl shadow-purple-900/5">
                        ...
                        <div class="flex flex-wrap gap-4 relative z-10">
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->url()) }}"
                                target="_blank"
                                class="inline-flex items-center gap-3 px-8 py-4 bg-[#1877F2]/10 border border-[#1877F2]/20 text-[#1877F2] rounded-2xl hover:bg-[#1877F2] hover:text-white transition-all duration-500 font-extrabold text-sm uppercase tracking-wider group-share shadow-sm hover:shadow-lg hover:shadow-[#1877F2]/20">
                                <i class="fab fa-facebook-f"></i>
                                Facebook
                            </a>
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($news->title) }}"
                                target="_blank"
                                class="inline-flex items-center gap-3 px-8 py-4 bg-[#1DA1F2]/10 border border-[#1DA1F2]/20 text-[#1DA1F2] rounded-2xl hover:bg-[#1DA1F2] hover:text-white transition-all duration-500 font-extrabold text-sm uppercase tracking-wider group-share shadow-sm hover:shadow-lg hover:shadow-[#1DA1F2]/20">
                                <i class="fab fa-twitter"></i>
                                Twitter
                            </a>
                            <a href="https://wa.me/?text={{ urlencode($news->title . ' ' . request()->url()) }}"
                                target="_blank"
                                class="inline-flex items-center gap-3 px-8 py-4 bg-[#25D366]/10 border border-[#25D366]/20 text-[#25D366] rounded-2xl hover:bg-[#25D366] hover:text-white transition-all duration-500 font-extrabold text-sm uppercase tracking-wider group-share shadow-sm hover:shadow-lg hover:shadow-[#25D366]/20">
                                <i class="fab fa-whatsapp"></i>
                                WhatsApp
                            </a>
                            <button onclick="copyToClipboard('{{ request()->url() }}')"
                                class="inline-flex items-center gap-3 px-8 py-4 bg-white border border-gray-200 text-gray-600 rounded-2xl hover:bg-gray-50 hover:text-[#2A1424] transition-all duration-500 font-extrabold text-sm uppercase tracking-wider group-share shadow-sm">
                    ...
                    {{-- Author Box --}}
                    <div class="bg-white border border-purple-100 rounded-[3rem] p-10 flex flex-col md:flex-row items-center gap-10 shadow-xl shadow-purple-900/5 group">
                        <div
                ...
                <aside class="lg:col-span-4 space-y-12">
                    {{-- Search Box --}}
                    <div class="bg-white border border-gray-200 rounded-[2.5rem] p-8 shadow-xl shadow-purple-900/5">
                        ...
                        <div class="grid gap-3">
                            ...
                            @foreach($categories as $cat)
                                <a href="{{ route('news') }}?category={{ $cat['name'] }}"
                                    class="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-[#932F80]/50 hover:bg-[#932F80]/10 transition-all group">
                    ...
                    @if(($settings['ppdb_active'] ?? false))
                       <div class="bg-gradient-to-br from-[#932F80] via-[#6E1F5F] to-[#2A1424] rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl group shadow-[#932F80]/20">
                ...
                <div class="grid md:grid-cols-3 gap-10">
                    @foreach($relatedNews as $related)
                        <a href="{{ route('news.show', $related->slug) }}"
                            class="group bg-white rounded-[3rem] overflow-hidden border border-gray-200 hover:border-[#932F80]/50 shadow-2xl shadow-purple-900/5 transition-all duration-700 hover:-translate-y-3">
