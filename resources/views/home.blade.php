...
<section class="relative min-h-screen flex items-center lg:items-end overflow-hidden bg-gradient-to-br from-[#612F73]/5 via-white to-[#F0E7F8]/30 pt-32 lg:pt-8 pb-0 lg:pb-20">
    ...
    <div class="container mx-auto px-6 lg:px-12 2xl:px-16 3xl:px-20 relative z-10 max-w-7xl 2xl:max-w-[1440px] 3xl:max-w-[1600px]">
        <div class="grid grid-cols-1 lg:grid-cols-2 items-center gap-14 2xl:gap-20 3xl:gap-24">
           ...
           {{-- RIGHT IMAGE --}}
<div class="relative flex items-end justify-center lg:justify-end h-full mt-2 lg:mt-0 lg:translate-y-20 px-4 lg:px-0">
...
<img
    ...
    class="absolute bottom-0 left-1/2 -translate-x-[55%]
           h-[95%] w-auto max-w-none
           object-contain object-bottom drop-shadow-[0_25px_40px_rgba(0,0,0,0.5)] 
           slider-item transition-all duration-1000 transform
           {{ $index === 0 ? 'opacity-100 translate-y-2 scale-100' : 'opacity-0 translate-y-10 scale-95' }}"
/>
        @endforeach
    </div>
                            ...
                            <div class="relative w-full aspect-[4/3] rounded-[1.5rem] overflow-hidden mb-6 group/image">
                                ...
                                @if($award->image)
                                    <img src="{{ asset('storage/' . $award->image) }}" alt="{{ $award->title }}"
                                        class="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700 ease-in-out" loading="lazy">
                                @else
                                    ...
                                    <div
                                        ...
                                        @if($bc->image)
                                            <img src="{{ asset('storage/' . $bc->image) }}" alt="{{ $bc->name }}"
                                                class="w-full h-full object-cover rounded-[1.75rem] group-hover:scale-110 transition-transform duration-700 ease-in-out" loading="lazy">
                                        @else
                                            <div
